...
const EditProductPage = () => {
    ...
    useEffect(() => {
        if (product) {
            const { _id, tags, image, __v, gallery, attributes, ...rest } =
                product;
            form.reset(rest);
            setImage(image);
            setGallery(gallery);
    ...
    const mutation = useMutation({
        mutationFn: (product: IProduct) => editProduct(id!, product),
        onSuccess: () => {
            form.reset();
            toast({
    ...
    const onSubmit: SubmitHandler<Inputs> = (product: IProduct) => {
        const newGallery = gallery.map((item) => (item ? item.url : null));
                ...
                >
                    ...
                    {gallery?.map((item, index) => {
                        return (
                            <div
                                key={index}
                                className="inline-block border mx-2"
                            >
                                <img
